  ...
  <div class="min-h-screen bg-neutral-50 dark:bg-neutral-950">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
        <div class="flex items-center gap-3 sm:gap-6">
          <router-link to="/" class="flex items-center gap-2 hover:opacity-80 transition-opacity">
            ...
            <div v-if="currentLogo && currentLogo.trim()" class="w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center overflow-hidden">
            ...
            <span class="text-base sm:text-lg font-bold text-neutral-900 dark:text-neutral-50">{{ siteSettings.site_title || 'AstraHook' }}</span>
          </router-link>
          <div class="hidden sm:block h-6 w-px bg-neutral-300 dark:bg-neutral-700"></div>
          <router-link to="/docs" class="hidden sm:flex items-center gap-2 hover:opacity-80 transition-opacity">
          ...
          <ChevronRight class="hidden sm:block w-4 h-4 text-neutral-400" />
          <span class="hidden sm:block text-sm font-medium text-neutral-900 dark:text-white">{{ page?.title }}</span>
        </div>
        <div class="flex items-center gap-2">
          ...
          <div v-if="authStore.isAuthenticated()" class="relative user-dropdown hidden sm:block">
            ...
            <div v-if="userMenuOpen"
              ...
              <router-link v-if="authStore.user?.role === 'admin'" to="/admin/dashboard" @click="userMenuOpen = false"
    ...
    <transition name="mobile-nav">
      <div v-if="mobileMenuOpen" class="lg:hidden fixed inset-0 z-50 bg-black/50 backdrop-blur-sm"
        ...
        <div
          class="absolute left-0 top-0 bottom-0 w-64 bg-white dark:bg-neutral-950 border-r border-neutral-200 dark:border-neutral-800/50 overflow-y-auto"
          @click.stop>
          <div class="p-4 border-b border-neutral-200 dark:border-neutral-800/50 flex items-center justify-between">
          ...
          <nav class="p-4 space-y-4">
            <div v-for="category in categories" :key="category.id">
              ...
              <div class="space-y-1 mt-2">
                <router-link v-for="navPage in pagesByCategory[category.id]" :key="navPage.id"
                  :to="`/docs/${navPage.slug}`" @click="mobileMenuOpen = false"
                  class="block px-3 py-2 text-sm rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800/50 transition-all"
                  :class="navPage.slug === $route.params.slug ? 'bg-neutral-100 dark:bg-neutral-800/50 text-neutral-900 dark:text-neutral-50 font-medium' : 'text-neutral-600 dark:text-neutral-400'">
                  {{ navPage.title }}
                </router-link>
              </div>
            </div>
    ...
    <div class="max-w-7xl mx-auto flex">
      ...
      <aside
        ...
        <nav class="p-4 space-y-4">
          <div v-for="category in categories" :key="category.id">
            <h3 class="px-3 py-2 text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
              {{ category.title }}
            </h3>
            <div class="space-y-1 mt-2">
              <router-link v-for="navPage in pagesByCategory[category.id]" :key="navPage.id"
                :to="`/docs/${navPage.slug}`"
                class="block px-3 py-2 text-sm rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800/50 transition-all"
                :class="navPage.slug === $route.params.slug ? 'bg-neutral-100 dark:bg-neutral-800/50 text-neutral-900 dark:text-neutral-50 font-medium' : 'text-neutral-600 dark:text-neutral-400'">
                {{ navPage.title }}
              </router-link>
      ...
      <main class="flex-1 px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div v-else-if="error" class="text-center py-12">
          ...
          <router-link to="/docs" class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">
            ← Back to Documentation
          </router-link>
        </div>

        <div v-else-if="page" class="max-w-4xl">
          <!-- Page Header -->
          <div class="mb-8">
            <h1 class="text-3xl sm:text-4xl font-bold text-neutral-900 dark:text-white mb-4">
              {{ page.title }}
            </h1>
          ...
          <div class="mt-12 pt-8 border-t border-neutral-200 dark:border-neutral-800">
            <div class="flex items-center justify-between">
              <router-link v-if="previousPage" :to="`/docs/${previousPage.slug}`"
                class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-neutral-600 hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-white transition-colors">
                <ArrowLeft class="w-4 h-4" />
                {{ previousPage.title }}
              </router-link>
              <div v-else></div>

              <router-link v-if="nextPage" :to="`/docs/${nextPage.slug}`"
                class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-neutral-600 hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-white transition-colors">
                {{ nextPage.title }}
...
import { BookOpen, Sun, Moon, Zap, Menu, X, ChevronRight, ArrowLeft, ArrowRight, ChevronDown } from 'lucide-vue-next'
...
const fetchPage = async (slug) => {
  try {
    ...
    const response = await api.get(`/api/docs/slug/${slug}`)
...
const previousPage = computed(() => {
  if (!page.value || !pages.value.length) return null
  const currentIndex = pages.value.findIndex(p => p.id === page.value.id)
  return currentIndex > 0 ? pages.value[currentIndex - 1] : null
})

const nextPage = computed(() => {
  if (!page.value || !pages.value.length) return null
  const currentIndex = pages.value.findIndex(p => p.id === page.value.id)
  return currentIndex < pages.value.length - 1 ? pages.value[currentIndex + 1] : null
})
...
<style scoped>
/* Same styles as original Docs.vue */
